<template>
	<u-popup v-model="value" mode="bottom" :popup="false" :mask="true" :closeable="true" :safe-area-inset-bottom="true"
	 close-icon-color="#ffffff" :z-index="uZIndex" :maskCloseAble="maskCloseAble" @close="close">
		<view class="title" v-if="titleText">{{titleText}}</view>
					...
					<view class="u-padding-10 u-bg-gray" style="height: 100%;">
						<scroll-view :scroll-y="true" style="height: 100%">
							<u-cell-group v-if="isChooseA">
								<u-cell-item v-for="(item,index) in streets" :title="item.label" :arrow="false" :index="index" :key="index"
								 @click="streetChange">
		...
		methods: {
			init() {
				if (this.areaCode.length == 3) {
					...
					this.setStreet("", this.areaCode[3]);
				} else if (this.defaultRegion.length == 3) {
					this.setProvince(this.defaultRegion[0], "");
					this.setCity(this.defaultRegion[1], "");
					this.setArea(this.defaultRegion[2], "");
					this.setStreet(this.defaultRegion[2], "");
				};
			},
